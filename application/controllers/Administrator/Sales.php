        ...
        if(isset($data->salesId) && $data->salesId != 0 && $data->salesId != ''){
            ...
            $saleDetails = $this->db->query("
                select 
                    ...
                    b.brand_name,
                    (
                        select sd.SaleDetails_TotalQuantity * p.sft
                    )as total_sft
                from tbl_saledetails sd
                join tbl_product p on p.Product_SlNo = sd.Product_IDNo
                join tbl_productcategory pc on pc.ProductCategory_SlNo = p.ProductCategory_ID
            ...
            $saleDetails = array_map(function($detail) {
                $detail->quantity_text = floor($detail->SaleDetails_TotalQuantity / $detail->box) .' box ' . $detail->SaleDetails_TotalQuantity % $detail->box . ' pcs';
                return $detail;
            }, $saleDetails);
        }
        $sales = $this->db->query("
