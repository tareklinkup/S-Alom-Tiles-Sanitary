...
class Products extends CI_Controller {
    ...
    public function addProduct() {
        ...
        try{
            ...
            $productNameCount = $this->db->query("select * from tbl_product where Product_Name = ? and Product_Code = ?", [$productObj->Product_Name, $productObj->Product_Code])->num_rows();
            if($productNameCount > 0){
                $res = ['success'=>false, 'message'=>'Product already exists'];
            ...
            if($productCodeCount > 0){
                $res = ['success'=>false, 'message'=>'Product model no already exists'];
                echo json_encode($res);
                exit;
            }
    ...
    public function getCurrentStock(){
        ...
        $stock = $this->db->query("
            select * from(
                select
                    ...
                    p.box,
                    (select (p.sft * current_quantity)) as total_sft,
                    (select 
                        CASE
                            WHEN 
                                p.convert_type = 'Sft' THEN p.Product_Purchase_Rate * total_sft
                            ELSE p.Product_Purchase_Rate * current_quantity
                        END AS stock_value
                    )as stock_value,
                    pc.ProductCategory_Name,
                    b.brand_name,
        ...
        $stock = array_map(function($product) {
            $product->quantity_text = floor($product->current_quantity / $product->box) .' box ' . $product->current_quantity % $product->box . ' pcs';
            return $product;
        }, $stock);
    ...
    public function getTotalStock(){
        ...
        $stock = $this->db->query("
            select
                ...
                (select current_quantity * p.sft)as total_sft,
                (select 
                    CASE
                        WHEN 
                            p.convert_type = 'Sft' THEN p.Product_Purchase_Rate * total_sft
