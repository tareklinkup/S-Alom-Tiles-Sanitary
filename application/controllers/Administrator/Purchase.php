        ...
        if(isset($data->purchaseId) && $data->purchaseId != null){
            ...
            $purchaseDetails = $this->db->query("
                select
                    ...
                    b.brand_name,
                    (
                        select pd.PurchaseDetails_TotalQuantity * p.sft
                    )as total_sft,
                    (
                        select p.sft * p.Product_Purchase_Rate
                    )as sft_amount
            ...
            $purchaseDetails = array_map(function($detail) {
                $detail->quantity_text = floor($detail->PurchaseDetails_TotalQuantity / $detail->box) .' box ' . $detail->PurchaseDetails_TotalQuantity % $detail->box . ' pcs';
                return $detail;
            }, $purchaseDetails);
    ...
    public function addPurchase()
    {
        ...
        try{
            ...
            foreach($data->cartProducts as $product){
                ...
                $p_total = ($product->purchaseRate * $product->quantity);
