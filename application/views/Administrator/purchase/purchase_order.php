						...
						<div class="col-sm-6">
							<form v-on:submit.prevent="addToCart">
								...
								<div class="form-group">
									...
									<div class="col-sm-3">
										<v-select v-bind:options="colors" v-model="color" label="color_name"></v-select>
									</div>
									<label class="col-sm-2 control-label no-padding-right">Brand</label>
									<div class="col-sm-3">
										<v-select v-bind:options="brands" v-model="brand" label="brand_name"></v-select>
									</div>
								</div>
								...
								<div class="form-group">
									<label class="col-sm-4 control-label no-padding-right"> Total Sft </label>
									<div class="col-sm-8">
										<input type="text"  class="form-control" readonly v-model="selectedProduct.totalSft"/>
									</div>
								</div>
				...
				<table class="table table-bordered" style="color:#000;margin-bottom: 5px;">
					<thead>
						<tr>
							...
							<th style="color:#000;">Sft</th>
							<th style="color:#000;">Box / Pcs</th>
					...
					<tbody style="display:none;" v-bind:style="{display: cart.length > 0 ? '' : 'none'}">
						<tr v-for="(product, sl) in cart">
							...
							<td>{{ parseFloat(product.sft).toFixed(2) }}</td>
						...
						<tr>
							<td colspan="9"><textarea style="width: 100%;font-size:13px;" placeholder="Note" v-model="purchase.note"></textarea></td>
							<td colspan="4" style="padding-top: 15px;font-size:18px;">{{ purchase.subTotal }}</td>
						</tr>
					</tbody>
				</table>
			</div>
	...
	new Vue({
		...
		watch: {
			selectedCategory(item) {
				...
				axios.post('/get_products', {isService: 'false', categoryId: item.ProductCategory_SlNo}).then(res=>{
					this.products = res.data;
				})
			},
			selectedProduct(product) {
				if(product == undefined) return;
				this.color = this.colors.find(item => item.color_SiNo == product.color);
				this.brand = this.brands.find(item => item.brand_SiNo == product.brand);
			}
		},
		created(){
			this.getBranches();
		...
		methods:{
			...
			productTotal(){
				this.selectedProduct.totalSft = parseFloat(this.selectedProduct.quantity * this.selectedProduct.sft).toFixed(2)
				this.selectedProduct.total =  this.selectedProduct.convert_type == 'Sft' ? parseFloat(this.selectedProduct.totalSft * this.selectedProduct.Product_Purchase_Rate).toFixed(2) : this.selectedProduct.quantity * this.selectedProduct.Product_Purchase_Rate;
			...
			addToCart(){
				...
				let quantityText = `${Math.floor(this.selectedProduct.quantity / this.selectedProduct.box)} box ${this.selectedProduct.quantity % this.selectedProduct.box} pcs`;


				let product = {
					productId: this.selectedProduct.Product_SlNo,
					model: this.selectedProduct.Product_Code,
					name: this.selectedProduct.Product_Name,
					color: this.selectedProduct.color_name,
					brand: this.selectedProduct.brand_name,
					...
					sft: this.selectedProduct.totalSft,
				...
				axios.post('/get_purchases', {purchaseId: this.purchase.purchaseId}).then(res => {
					...
					r.purchaseDetails.forEach(product => {
						let cartProduct = {
							...
							sft: product.total_sft,
							totalSft: this.selectedProduct.convert_type == 'Sft' ? parseFloat(this.selectedProduct.Product_Purchase_Rate * this.selectedProduct.totalSft).toFixed(2): ''
						}
